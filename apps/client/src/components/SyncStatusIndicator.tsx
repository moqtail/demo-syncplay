...
export function SyncStatusIndicator({
  ...
  const getSyncStatusText = () => {
    ...
    if (role === 'leader') {
      ...
      if (Math.abs(syncDelta) < threshold) return '✓ In Sync';
      return '❌ Out of Sync';
    }
  };
  ...
  const getSyncStatusColor = () => {
    ...
    if (role === 'leader') {
      ...
      if (Math.abs(syncDelta) < threshold) return '#28a745';
      return '#dc3545';
    }
  };
      ...
      >
        ...
        {role === 'follower' && syncDelta !== undefined && (
          <span style={{ fontSize: '12px', color: '#6c757d', marginLeft: '8px' }}>
            {syncDelta > 0 ? '+' : ''}{(syncDelta * 1000).toFixed(0)}ms
          </span>
        )}
        {role === 'follower' && (
          <div ref={settingsRef} style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center' }}>
            ...
            {showSettings && (
              ...
              >
                <div style={{ marginBottom: '8px', fontSize: '12px', fontWeight: '600', color: '#495057' }}>
                  Max Allowed Drift
                </div>
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '14px', fontWeight: '600', minWidth: '60px', textAlign: 'center', color: '#495057' }}>
