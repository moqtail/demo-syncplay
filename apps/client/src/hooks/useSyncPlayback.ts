...
export function useSyncPlayback({
  ...
  const deltaLogsRef = useRef<[number, number, number][]>([]);
  ...
  const handleSyncUpdate = useCallback(
    (message: SyncUpdateMessage) => {
      ...
      const delta = currentTime - leaderTime; // Signed: positive if ahead, negative if behind

      syncDeltaRef.current = delta;
      setSyncDelta(delta);
      lastSyncTimeRef.current = Date.now();
      const isForceSeek = Math.abs(delta) > deltaThresholdRef.current;
      deltaLogsRef.current.push([Date.now(), delta, isForceSeek ? 1 : 0]);

      console.log(`[SyncPlayback] Follower sync update: delta=${delta.toFixed(3)}s, threshold=${deltaThresholdRef.current.toFixed(3)}s, needsSync=${isForceSeek}`);
  ...
  const downloadDeltaLogs = useCallback(() => {
    if (deltaLogsRef.current.length === 0) {
      alert('No sync logs to download');
      return;
    }

    const csvContent = "data:text/csv;charset=utf-8,"
      + deltaLogsRef.current.map(e => e.join(",")).join("\n");

    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `sync_deltas_${Date.now()}.csv`);
