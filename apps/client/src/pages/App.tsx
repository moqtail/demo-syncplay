...
function App() {
  ...
  const [isWebTransportSupported, setIsWebTransportSupported] = useState<boolean>(true)
  ...
  // Check WebTransport support on mount
  useEffect(() => {
    if (typeof WebTransport === 'undefined') {
      setIsWebTransportSupported(false)
    } else {
      setIsWebTransportSupported(true)
    }
  }, [])
  ...
  const enqueueAppend = useCallback((data: Uint8Array) => {
    ...
    if (!sb) {
      console.warn('[App] SourceBuffer not ready yet, dropping data')
      return
    }
    ...
    const maybeAppend = () => {
      ...
      try {
        ...
        console.error('[App] appendBuffer error', e)
  ...
  const cleanupBuffer = useCallback((currentTimeSeconds: number) => {
    ...
    try {
      ...
      if (first.end <= mainStart) {
        ...
        removeEnd = first.end
        // console.log(
        //   `[App] cleanupBuffer: removing fully old range [${removeStart.toFixed(2)}, ${removeEnd.toFixed(2)}], mainStart=${mainStart.toFixed(2)}`
        // )
      }
        ...
        removeEnd = last.end
        // console.log(
        //   `[App] cleanupBuffer: removing fully future range [${removeStart.toFixed(2)}, ${removeEnd.toFixed(2)}], mainEnd=${mainEnd.toFixed(2)}`
        // )
      }
      else {
        if (first.start < mainStart && first.end > mainStart) {
          ...
          removeEnd = mainStart
          // console.log(
          //   `[App] cleanupBuffer: trimming old tail [${removeStart.toFixed(2)}, ${removeEnd.toFixed(2)}] from [${first.start.toFixed(2)}, ${first.end.toFixed(2)}]`
          // )
        } else if (last.start < mainEnd && last.end > mainEnd) {
          removeStart = mainEnd
          removeEnd = last.end
          // console.log(
          //   `[App] cleanupBuffer: trimming future head [${removeStart.toFixed(2)}, ${removeEnd.toFixed(2)}] from [${last.start.toFixed(2)}, ${last.end.toFixed(2)}]`
          // )
        } else {
          ...
          if (removeEnd > removeStart) {
            console.warn(
              `[App] cleanupBuffer: fallback trimming [${removeStart.toFixed(2)}, ${removeEnd.toFixed(2)}] due to tight budget`
            )
          } else {
            removeStart = null
            removeEnd = null
      ...
      if (removeStart != null && removeEnd != null && removeEnd > removeStart) {
        try {
          ...
          console.error('[App] cleanupBuffer: sb.remove failed', e)
        }
      }
    } catch (e) {
      console.error('[App] Error in cleanupBuffer:', e)
    }
  }, [
  ...
  const handleSyncRequired = useCallback((targetTime: number, targetGroup: number) => {
    if (userRole !== 'follower') {
      return
    }

    const prevLeader = leaderPositionRef.current
    const prevGroup = prevLeader?.group ?? null

    leaderPositionRef.current = { time: targetTime, group: targetGroup }

    if (prevGroup === null) {
      const desiredStart = Math.max(1, targetGroup)
      if (nextGroupRef.current < desiredStart) {
        console.log(
          `[App] [Sync] Initial steering nextGroup from ${nextGroupRef.current} -> ${desiredStart} (targetGroup=${targetGroup})`
        )
        nextGroupRef.current = desiredStart
      }
      return
    }

    const deltaGroups = targetGroup - prevGroup
    const GROUPS_PER_SECOND = settings.groupsPerSecond
    const SEEK_THRESHOLD_GROUPS = settings.syncMaxAllowedDriftMs / 1000 * GROUPS_PER_SECOND

    if (deltaGroups < -SEEK_THRESHOLD_GROUPS) {
      console.log(
        `[App] [Sync] Detected backward seek: leader group ${prevGroup} -> ${targetGroup} (Δ=${deltaGroups} groups)`
      )
        ...
        try {
          if (!sb.updating) {
            ...
            if (buffered.length > 0) {
              ...
              console.log(`[App] [Sync] Clearing MSE buffer [${start.toFixed(2)}, ${end.toFixed(2)}]`)
              sb.remove(start, end)
            }
            } else {
            console.warn('[App] [Sync] SourceBuffer is updating, cannot clear immediately on seek')
          }
        } catch (e) {
          console.error('[App] [Sync] Failed to clear SourceBuffer on backward seek:', e)
        }
      }

      bufferedRangesRef.current.clear()
      appendQueueRef.current = []
      appendingRef.current = false

      const BACK_BUFFER_GROUPS = GROUPS_PER_SECOND // ~1 second of backbuffer
      const startGroup = Math.max(1, targetGroup - BACK_BUFFER_GROUPS)
      nextGroupRef.current = startGroup

      console.log(
        `[App] [Sync] Reset bufferedRanges and nextGroup to ${startGroup} for backward seek to ${targetGroup}`
      )

      // Streaming loop will handle fetching now
      return
    }

    // ----- SMALL DRIFT / FORWARD MOVEMENT -----
    const fetchAheadGroups = settings.fetchAheadSeconds * GROUPS_PER_SECOND
    const desiredStart = Math.max(1, targetGroup)
    const desiredEnd = targetGroup + fetchAheadGroups

    // If we are behind the sync window, pull nextGroup forward
    if (nextGroupRef.current < desiredStart) {
      console.log(
        `[App] [Sync] Steering nextGroup from ${nextGroupRef.current} -> ${desiredStart} (targetGroup=${targetGroup})`
      )
      nextGroupRef.current = desiredStart
    }

    // If we are oddly far ahead of the sync window (e.g. after forward over-fetch),
    // optionally clamp back inside the window as well:
    if (nextGroupRef.current > desiredEnd) {
      console.log(
        `[App] [Sync] Clamping nextGroup from ${nextGroupRef.current} -> ${desiredEnd} (targetGroup=${targetGroup})`
      )
      nextGroupRef.current = desiredEnd
    }
  }, [settings.syncMaxAllowedDriftMs, userRole, settings.fetchAheadSeconds, settings.groupsPerSecond])


  const { syncDelta, downloadDeltaLogs } = useSyncPlayback({
    syncService,
    role: userRole,
    videoRef,
    syncConfig,
  ...
  const handleSubscribeClick = async () => {
    try {
      ...
      console.error('[App] Subscribe failed', err)
      setRequestState({
        isLoading: false,
  ...
  const handleJoinRoom = useCallback(async (roomId: string, role: UserRole, userName: string, videoId: string) => {
    ...
    try {
      ...
      await handleJoinRoomClick(role)
  ...
  const handleLeaveRoom = useCallback(() => {
    ...
    if (mediaSourceRef.current && mediaSourceRef.current.readyState === 'open') {
      try {
        ...
        console.warn('[App] Failed to end MediaSource:', e)
      }
    }
  ...
  const handleJoinRoomClick = async (role: UserRole) => {
    try {
      setRequestState({ isLoading: true, error: null });
      streamingRef.current = true;

      if (role === 'follower') {
        if (leaderPositionRef.current) {
          nextGroupRef.current = leaderPositionRef.current.group + 1;
        } else if (initialPlaybackStateRef.current) {
          nextGroupRef.current = timeToGroup(initialPlaybackStateRef.current.timestamp);
        } else {
          ...
          try {
            ...
            (async () => {
              try {
                while (streamingRef.current) {
                  ...
                  if (userRole === 'follower') {
                    if (leaderPositionRef.current) {
                      ...
                      currentTimeSeconds = initialPlaybackStateRef.current.timestamp;
                  ...
                  if (userRole === 'follower' && leaderPositionRef.current && !hasJumpedAheadRef.current && currentGroup > 1) {
                    const jumpToGroup = Math.max(1, currentGroup);
                    console.log(`[App] [Follower] Initial jump to leader position: nextGroup ${nextGroupRef.current} → ${jumpToGroup} (leader at group ${currentGroup})`);
                    nextGroupRef.current = jumpToGroup;
                    hasJumpedAheadRef.current = true;
                  }

                  // If nextGroup is far behind (e.g. after a user seek), skip ahead
                  const desiredMin = Math.max(1, currentGroup - 1)
                  if (nextGroupRef.current < desiredMin) {
                    console.log(`[App] Skipping ahead: nextGroup ${nextGroupRef.current} -> ${desiredMin}`)
                    nextGroupRef.current = desiredMin
                  }

                  // Fetch a small batch starting from nextGroup
                  const targetGroup = nextGroupRef.current;
                  const endGroup = Math.min(targetGroup + 5, maxGroupToFetch);

                  if (!isRangeBuffered(targetGroup, endGroup)) {
                    console.log(`[App] Fetching groups ${targetGroup} to ${endGroup}`);
                    ...
                    try {
                      ...
                      markGroupsBuffered(targetGroup, endGroup);
                      nextGroupRef.current = endGroup + 1;
                    } catch (err) {
                      console.error(`[App] Error fetching groups ${targetGroup} to ${endGroup}:`, err);
                    }
                  } else {
                    nextGroupRef.current = endGroup + 1;
                  }
                ...
                console.error('[App] Streaming loop failed', err);
                setRequestState({
                  isLoading: false,
                  error: err instanceof Error ? err.message : String(err),
                });
                ...
                try {
                  ...
                  console.error('[App] Failed to end stream:', e);
                }
              }
            })();
          } catch (e) {
            reject(e);
            console.error('[App] Error in sourceopen handler', e);
            setRequestState({
              isLoading: false,
            ...
            try {
              ...
              console.error('[App] Failed to end stream:', endErr);
            }
          }
        });
  ...
  const handleSeeking = useCallback(async () => {
    if (userRole === 'follower' && !isIndependentMode) {
      return
    }
    const video = videoRef.current
    ...
    const fetchAheadGroups = settings.fetchAheadSeconds * settings.groupsPerSecond
    let endGroup = seekGroup + fetchAheadGroups - 1

    const startGroup = seekGroup
    endGroup = Math.min(seekGroup + 5, endGroup)

    if (!isRangeBuffered(startGroup, endGroup)) {
      isSeeking.current = true
      setRequestState({ isLoading: true, error: null })
      try {
        console.log(`[App] Fetching groups ${startGroup} to ${endGroup} for seek`)
        nextGroupRef.current = startGroup
        // Advance nextGroup immediately to avoid the streaming loop fetching overlapping ranges
        nextGroupRef.current = Math.max(nextGroupRef.current, endGroup + 1)
        ...
        markGroupsBuffered(startGroup, endGroup)

        setRequestState({ isLoading: false, error: null })
      } catch (err) {
        console.error(`[App] Seek fetch failed for groups ${startGroup} to ${endGroup}:`, err)
        setRequestState({
          isLoading: false,
          error: err instanceof Error ? err.message : String(err),
        })
      ...
      console.log(`[App] Seek target already buffered`)
      nextGroupRef.current = Math.max(nextGroupRef.current, seekGroup + fetchAheadGroups)
    }
  }, [userRole, isIndependentMode, settings, timeToGroup, isRangeBuffered, enqueueAppend, markGroupsBuffered])
  ...
  useEffect(() => {
    ...
    //console.log('[App] Attaching video event listeners')
    video.addEventListener('seeking', handleSeeking)
    video.addEventListener('timeupdate', handleTimeUpdate)

    return () => {
      //console.log('[App] Removing video event listeners')
    ...
    return () => {
      ...
      if (mediaSourceRef.current && mediaSourceRef.current.readyState === 'open') {
        try {
          ...
          console.error('[App] Failed to end stream on cleanup:', e)
        }
      }
      disconnectMOQ().catch((e) => console.error('[App] error in disconnectMOQ', e))
    }
  }, [])
  ...
  return (
    <div className="mp4-requester-container">
      ...
      {currentPage === 'lobby' ? (
        <div className="mp4-requester-lobby-wrapper">
          <div className="mp4-requester-lobby">
            <div className="mp4-requester-logo">
              <img src="/moqtail.svg" alt="MOQtail Logo" />
            </div>

            <div className="mp4-requester-content">
              <h1 className="mp4-requester-title">SyncPlay: Synchronized VoD with MOQ</h1>

              {/* WebTransport Warning */}
              {!isWebTransportSupported && (
                <div className="compatibility-error" style={{ marginBottom: '20px' }}>
                  <strong>⚠️ WebTransport Not Supported</strong>
                  <p>
                    This application requires WebTransport support. Please enable it.
                  </p>
                </div>
              )}
              ...
              >
                <button
                  ...
                  disabled={requestState.isLoading || relaySubscribed || !isWebTransportSupported}
                  className="mp4-requester-button"
                  style={{
                    backgroundColor: relaySubscribed ? '#28a745' : '#577B9F',
                    cursor: relaySubscribed || !isWebTransportSupported ? 'default' : 'pointer',
                    opacity: relaySubscribed || !isWebTransportSupported ? 0.7 : 1,
                  }}
                >
              ...
              <RoomControls
                ...
                rooms={rooms}
                config={serverConfig}
                disabled={requestState.isLoading || !syncService || !isWebTransportSupported}
              />
            </div>
          </div>
        </div>
        ...
        <>
          ...
          {isIndependentMode && (
            <div style={{
              ...
              <div style={{ flex: '1', minWidth: '200px', color: '#856404' }}>
